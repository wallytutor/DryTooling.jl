...
struct SimulationResiduals
    ...
    Manage iterative solvers residuals storage during a simulation. The memory is
    initialized with a given number of inner and outer iterations and resizing is
    not under the scope of this structure.
...
function step(r::SimulationResiduals, ε::Vector{Float64})::Nothing
    """
...
function equidistantcellsgrid(R::Float64, N::Int64)::UserDefinedGrid1D
    """
...
function geometriccellsgrid(R::Float64, N::Int64)::UserDefinedGrid1D
    """
...
struct TridiagonalProblem <: AbstractMatrixProblem
    ...
    Memory for a tridiagonal problem of rank `N`. All tensors are filled
    with zeros upon creation. This is simply a utility for memory allocation.
...
function step!(
    ...
    coefficients and solution is performed through user-supplied functions.
    
    The logic of this method works as:
    
    1. The `fouter!` update function is called once. This is generall
       where one implements the right-hand side update of the problem
       before stepping.
    
    2. An initial update with `finner!` is done. Normally this is
       responsible by the update of matrix coefficients that are dependent
       on solution state.
    
    3. If relaxation `α <= 0.0`, then the problem is treated as linear.
    
    4. Otherwise a maximum of `M` iterations are repeated, where `fsolve!`
       is used to solve the problem (ofter an under-relaxation step) and
       is also expected to keep track of residuals.
...
function fouter!(::AbstractDiffusionModel1D, ::Float64, ::Int64)
    @error "An specialization of this method is expexted!"
end

function finner!(::AbstractDiffusionModel1D, ::Float64, ::Int64)
    @error "An specialization of this method is expexted!"
end

function fsolve!(::AbstractDiffusionModel1D, ::Float64, ::Int64, ::Float64)
    @error "An specialization of this method is expexted!"
end
...
function timepoints(m::AbstractDiffusionModel1D)
...
################################################################################
# DIFFUSION Cylinder1DTemperatureModel
################################################################################
...
################################################################################
# DIFFUSION Sphere1DTemperatureModel
################################################################################
